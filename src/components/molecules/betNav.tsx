...
function BetNavBar({ slotName, titleBar, setView, view }: any) {
  return (
    <Nav className='bg-black flex space-x-4 items-center rounded-t-2xl justify-between text-sm w-[100%] py-2 px-4 h-14'>
      ...
      <div className='flex justify-between items-center space-x-4'>
        <span className='flex justify-between items-center space-x-2 rounded-full'>
          <IconChevronLeft className='bg-kpm w-5 h-5 rounded-full cursor-pointer' />
          <IconChevronRight className='bg-kpm w-5 h-5 rounded-full cursor-pointer' />
        </span>
        <Button
          loader={false}
          onClick={() => setView({ name: slotName, status: !view.status })}
          className={cn(
